...
import type { NewBookingData, BookingCreateData, BookingStatus, BookingUpdates, DbResult, Booking } from '../types';
...
export function useBookingActions() {
  ...
  const resolveBookingStatus = useCallback((userId: string): BookingStatus => {
  ...
  const handleApprove = useCallback(async (id: string, { singleOnly = false } = {}) => {
  ...
  const handleReject = useCallback(async (id: string, { singleOnly = false } = {}) => {
    const booking = bookings.find(b => b.id === id);
    if (!booking) return;
    const result = (booking.seriesId && !singleOnly)
  ...
  const handleNewBooking = useCallback(async (data: NewBookingData) => {
    if (!data.userId) {
    ...
    const newBookings: BookingCreateData[] = data.dates.map(date => ({
  ...
  const handleEditBooking = useCallback(async (bookingId: string, updates: BookingUpdates): Promise<DbResult<Booking>> => {
    const booking = bookings.find(b => b.id === bookingId);
    if (!booking) return { data: null, error: 'Buchung nicht gefunden' };
  ...
  const handleDeleteBooking = useCallback(async (bookingId: string, deleteType: string, seriesId?: string | null) => {

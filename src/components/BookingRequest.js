      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className={sectionCls}>
            ...
            <div className="max-h-[300px] overflow-y-auto flex flex-col gap-1.5">
              {previewDates.map(date => {
                ...
                return (
                  <div key={date} className={`px-3 py-2.5 rounded-lg border-2 ${
                    hasError ? 'border-red-300 bg-red-50' : hasConflict ? 'border-yellow-300 bg-yellow-50' : 'border-green-300 bg-green-50'
                  }`}>
                    <div className="flex justify-between items-center">
                      ...
                      {!hasConflict && <span className="text-xs text-green-600 font-semibold">Verfügbar</span>}
                    </div>
        ...
        <button type="submit"
          ...
          className={`w-full py-3 px-6 rounded-lg text-[15px] font-bold border-none flex items-center justify-center gap-2 transition-colors ${
            conflictAnalysis.hasErrors || previewDates.length === 0 || !formData.resourceId

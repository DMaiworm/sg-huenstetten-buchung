...
import type { BookingType, DateHolidayInfo, User } from '../types';
...
interface DateStatus extends DateHolidayInfo {
  excluded: boolean;
  manuallySet: boolean;
}

const BookingRequest: React.FC<BookingRequestProps> = ({ onSubmit }) => {
  ...
  const allowedEventTypes = useMemo(() => {
    if (!(selectedTeam as any)?.eventTypes) return EVENT_TYPES;
    return EVENT_TYPES.filter(et => (selectedTeam as any).eventTypes.includes(et.id));
  }, [selectedTeam]);
  ...
  const conflictAnalysis = useMemo(() => {
    if (previewDates.length === 0 || !formData.startTime || !formData.endTime || !formData.resourceId) {
      return { conflicts: [] as { date: string; conflicts: { severity: string; message: string; explanation?: string }[] }[], hasErrors: false, hasWarnings: false };
    }
    const conflicts = checkBookingConflicts(
      formData.resourceId, previewDates, formData.startTime, formData.endTime,
      formData.eventType as BookingType, bookings, slots, resources,
    );
    return {
      conflicts,
  ...
  const dateHolidayMap = useMemo(() => {
    const map: Record<string, DateHolidayInfo> = {};
  ...
  const getDateStatus = (date: string): DateStatus => {
  ...
  const toggleDateOverride = (date: string) => {
  ...
  const handleFacilityChange = (v: string) => setFormData(p => ({ ...p, facilityId: v, groupId: '', resourceId: '' }));
  const handleGroupChange = (v: string) => setFormData(p => ({ ...p, groupId: v, resourceId: '' }));
  const handleClubChange = (v: string) => setFormData(p => ({ ...p, clubId: v, departmentId: '', teamId: '', eventType: 'training' }));
  const handleDeptChange = (v: string) => setFormData(p => ({ ...p, departmentId: v, teamId: '', eventType: 'training' }));
  const handleTeamChange = (v: string) => {
    const t = (teams || []).find(x => x.id === v);
    setFormData(p => ({ ...p, teamId: v, eventType: (t as any)?.eventTypes?.[0] || 'training' }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      isComposite, includedResources: isComposite ? (resource as any).includes : null,
    });
      ...
      <form onSubmit={handleSubmit}>
          ...
          {isComposite && (
            <div className="mt-3 px-3 py-2.5 bg-blue-50 rounded-lg flex items-center gap-2 text-[13px] text-blue-800">
              ...
              <span><strong>Reserviert automatisch:</strong>{' '}
                {resources.filter(r => ((resource as any).includes || []).includes(r.id)).map(r => r.name).join(', ')}
              </span>
            </div>
          )}
          {isLimited && (
        ...
        <div className={sectionCls}>
          ...
          <div className="flex gap-2 mb-4">
            {([{ mode: 'single', label: 'Einzeltermin', Icon: CalendarDays }, { mode: 'series', label: 'Terminserie', Icon: CalendarRange }] as const).map(({ mode, label, Icon }) => (
          ...
          <div className={sectionCls}>
            ...
            <div className="max-h-[400px] overflow-y-auto flex flex-col gap-1.5">
              {previewDates.map(date => {
                ...
                let borderCls: string, bgCls: string;
                if (isExcluded) {

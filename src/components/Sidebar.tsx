...
const Sidebar: React.FC<SidebarProps> = ({ pendingCount, kannBuchen, kannGenehmigen, kannVerwalten, kannAdministrieren, istTrainer, open, onClose }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 flex flex-col h-full transition-transform duration-200 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 md:transition-none`}>
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <button onClick={onClose} className="md:hidden p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100" aria-label="Menü schließen">
            <X className="w-5 h-5" />
          </button>
        </div>

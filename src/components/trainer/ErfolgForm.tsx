...
interface ErfolgData {
  jahr: number;
  mannschaft: string;
  titel: string;
}

const EMPTY: ErfolgData = { jahr: new Date().getFullYear(), mannschaft: '', titel: '' };

interface ErfolgFormProps {
  initial?: ErfolgData | null;
  onSave: (data: { jahr: number; mannschaft: string; titel: string }) => void;
  onCancel: () => void;
  loading: boolean;
}

const ErfolgForm: React.FC<ErfolgFormProps> = ({ initial = null, onSave, onCancel, loading }) => {
  const [form, setForm] = useState<ErfolgData>(initial || EMPTY);

...
interface LizenzFormProps {
  initial?: LizenzData | null;
  onSave: (data: { bezeichnung: string; ausstellendeOrg: string | null; ausstellungsdatum: string | null; ablaufdatum: string | null }) => void;
  onCancel: () => void;
  loading: boolean;
}

const LizenzForm: React.FC<LizenzFormProps> = ({ initial = null, onSave, onCancel, loading }) => {
  const [form, setForm] = useState<LizenzData>(initial || EMPTY);
    ...
    <form onSubmit={handleSubmit} className="bg-gray-50 rounded-lg p-4 space-y-3 border border-gray-200">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2">
          ...
          <input type="text" value={form.bezeichnung} onChange={e => set('bezeichnung', e.target.value)}
        ...
        <div>
          ...
          <input type="text" value={form.ausstellendeOrg} onChange={e => set('ausstellendeOrg', e.target.value)}
            placeholder="z.B. DFB"
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <input type="date" value={form.ausstellungsdatum || ''} onChange={e => set('ausstellungsdatum', e.target.value || null)}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Gültig bis</label>
            <input type="date" value={form.ablaufdatum || ''} onChange={e => set('ablaufdatum', e.target.value || null)}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
        </div>
      </div>

...
const PDFExportPage: React.FC = () => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      for (let week = 0; week < numWeeks; week++) {
        for (let dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
          ...
          if (cellIndex >= firstDay && currentDay <= daysInMonth) {
            ...
            catResources.forEach((res: BookableResource, i: number) => {
            ...
            catResources.forEach((res: BookableResource, resIndex: number) => {
              ...
              dayBookings.forEach(booking => {
                if (yOffset + blockHeight <= weekHeight - 1) {
                  ...
                  const teamShort = (org?.team as any)?.shortName || teamFull;
                  doc.setFontSize(3.5);
                  doc.setFont('helvetica', 'bold');
                  ...
                  const rgb = hexToRgb((org?.team as any)?.color || res.color);
      ...
      catResources.forEach((res: BookableResource) => {
    ...
    <div>
      <div className="mb-6 flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={effectiveFacilityId} onChange={e => handleFacilityChange(e.target.value)}
        ...
        <div className="flex items-center gap-2">
          <Building2 className="w-4 h-4 text-gray-400 flex-shrink-0" />
          <select value={effectiveCategory} onChange={e => setSelectedCategory(e.target.value)}
            className="px-3 py-1.5 text-sm font-semibold bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 cursor-pointer">

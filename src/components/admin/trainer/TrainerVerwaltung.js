...
/**
 * TrainerVerwaltung – Admin-Seite /admin/trainer
 *
 * Zeigt alle Trainer mit Vollständigkeits-Ampel.
 * Filter-Tabs nach Operator (Verein).
 * Super-Admin sieht alle; Verwalter mit operator_id nur eigenen Verein.
 */
export default function TrainerVerwaltung({ operators = [] }) {
  ...
  // Tab-Filterung (nur Super-Admin bekommt mehrere Tabs)
  ...
  const vollst     = displayedTrainers.filter(t => t.details?.unterlagenVollstaendig).length;
  const fzVerified = displayedTrainers.filter(t => t.details?.fuehrungszeugnisVerified).length;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={`${total} Trainer${total !== 1 ? '' : ''} · ${vollst} vollständig · ${fzVerified} FZ verifiziert`}
        actions={
          <button
            onClick={refresh}

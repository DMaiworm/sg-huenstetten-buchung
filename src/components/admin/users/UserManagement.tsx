...
const UserManagement: React.FC = () => {
  ...
  const handleEdit = (user: User) => { setNewUser({ ...user } as any); setEditingUser(user); setShowForm(true); };
  ...
  const getAssignedResources = (userId: string) =>
    (genehmigerAssignments || []).filter((a: any) => a.user_id === userId).map((a: any) => a.resource_id);

  const handleToggleResource = async (userId: string, resourceId: string, isAssigned: boolean) => {
  ...
  const resourceTree = (facilities || []).map(fac => ({
    ...
    groups: (resourceGroups || []).filter(g => g.facilityId === fac.id).map(grp => ({
      ...grp,
      resources: (resources || []).filter((r: any) => r.groupId === grp.id),
    })),
  }));

...
interface FacilitySectionProps {
  ...
  onAddGroup: (facilityId: string) => void;
...
const FacilitySection: React.FC<FacilitySectionProps> = ({ facility, groups, resources, onUpdateFacility, onDeleteFacility, onUpdateGroup, onDeleteGroup, onAddGroup, onUpdateResource, onDeleteResource, onAddResource, slots, onAddSlot, onDeleteSlot }) => {
  const [expanded, setExpanded] = useState(true);
  const [editing, setEditing] = useState(false);
  const facilityGroups = groups.filter(g => g.facilityId === facility.id).sort((a, b) => a.sortOrder - b.sortOrder);
  const facilityResourceCount = resources.filter((r: any) => facilityGroups.some(g => g.id === r.groupId)).length;

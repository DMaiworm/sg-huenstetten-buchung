...
import type { Slot, SubResource } from '../../../types';
...
interface ResourceCardProps {
  ...
  showSlots: boolean;
  slots?: Slot[];
  onAddSlot?: (slot: { dayOfWeek: number; startTime: string; endTime: string; validFrom: string; validUntil: string; resourceId: string }) => Promise<void>;
  onDeleteSlot?: (id: string) => Promise<void>;
}

const ResourceCard: React.FC<ResourceCardProps> = ({ resource, onUpdate, onDelete, showSlots, slots, onAddSlot, onDeleteSlot }) => {
  ...
  const handleSubUpdate = (idx: number, updated: SubResource) => {
  ...
  const handleToggleSplittable = (val: boolean) => {
  ...
  return (
    <div className="border border-gray-200 rounded-lg bg-white mb-2 overflow-hidden">
      ...
      {showSlots && slotsOpen && slots && onAddSlot && onDeleteSlot && (

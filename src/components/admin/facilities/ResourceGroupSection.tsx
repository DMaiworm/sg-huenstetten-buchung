...
import type { ResourceGroup, Slot } from '../../../types';

interface ResourceGroupSectionProps {
  group: ResourceGroup;
  resources: any[];
  onUpdateGroup: (g: ResourceGroup) => void;
  onDeleteGroup: () => void;
  onUpdateResource: (id: string, updated: any) => void;
  onDeleteResource: (id: string) => void;
  onAddResource: (groupId: string) => void;
  slots: Slot[];
  onAddSlot: (slot: any) => Promise<void>;
  onDeleteSlot: (id: string) => Promise<void>;
}

const ResourceGroupSection: React.FC<ResourceGroupSectionProps> = ({ group, resources, onUpdateGroup, onDeleteGroup, onUpdateResource, onDeleteResource, onAddResource, slots, onAddSlot, onDeleteSlot }) => {
  const [expanded, setExpanded] = useState(true);
  const [editingName, setEditingName] = useState(false);
  const groupIcon = GROUP_ICONS.find(g => g.id === group.icon);
  const groupResources = resources.filter((r: any) => r.groupId === group.id);
  ...
  return (
    <div className="mb-3">
      <div className="flex items-center gap-2 mb-2 p-2 bg-gray-50 rounded-lg">
        ...
        <select value={group.icon} onChange={e => onUpdateGroup({ ...group, icon: e.target.value as any })} className="text-xs border border-gray-200 rounded px-1.5 py-1">

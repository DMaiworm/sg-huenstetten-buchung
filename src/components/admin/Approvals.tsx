...
interface SeriesContainer extends Booking {
...
interface ApprovalsProps {
  onApprove: (bookingId: string, opts?: { singleOnly?: boolean }) => Promise<void>;
  onReject: (bookingId: string, opts?: { singleOnly?: boolean }) => Promise<void>;
}

const Approvals: React.FC<ApprovalsProps> = ({ onApprove, onReject }) => {
  ...
  const groupedPending = useMemo(() => {
    const seriesMap: Record<string, SeriesContainer> = {};
    const singles: Booking[] = [];
    pendingBookings.forEach(b => {
      if (b.seriesId) {
        if (!seriesMap[b.seriesId]) {
          seriesMap[b.seriesId] = { ...b, seriesBookings: [], totalCount: 0, freeCount: 0, blockedCount: 0 } as SeriesContainer;
        }
    ...
    return [...Object.values(seriesMap), ...singles] as (SeriesContainer | Booking)[];
  }, [pendingBookings, bookings]);

  const getUserInfo = (userId: string) => {
    const user = users.find(u => u.id === userId);
    if (!user) return { name: 'Unbekannt', role: null as any };
    const role = ROLES.find(r => r.id === (user as any).role);
  ...
  const fmtDateShort = (s: string) => new Date(s).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit' });

  const handleApproveWrapped = async (bookingId: string, opts?: { singleOnly?: boolean }) => {
  ...
  const handleReject = async (bookingId: string, opts?: { singleOnly?: boolean }) => {
    if (processingId) return;
      ...
      {groupedPending.length === 0 ? (
        ...
        <div className="space-y-4">
          {groupedPending.map(item => {
            const isSeries = 'seriesBookings' in item && item.seriesBookings && item.seriesBookings.length > 1;
            ...
            if (isSeries) {
              const seriesItem = item as SeriesContainer;
              const dateRange = fmtDate(seriesItem.seriesBookings[0].date) + ' – ' + fmtDate(seriesItem.seriesBookings[seriesItem.seriesBookings.length - 1].date);
              const dayName = DAYS_FULL[new Date(seriesItem.seriesBookings[0].date).getDay()];

              return (
                <div key={seriesItem.seriesId} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                  <div className="p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            <button onClick={() => toggleSeries(seriesItem.seriesId!)}
                            ...
                            <h3 className="font-semibold text-gray-800">{seriesItem.title}</h3>
                            <span className="inline-flex items-center gap-1 text-xs text-blue-600 font-semibold">
                              <Repeat className="w-3 h-3" />{seriesItem.totalCount}x Serie
                            </span>
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-gray-600 flex items-center gap-2"><Clock className="w-4 h-4" />{seriesItem.startTime} - {seriesItem.endTime}</p>
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <span className="inline-flex items-center gap-1 text-xs font-medium text-green-700 bg-green-50 px-2 py-0.5 rounded-full">
                              <CheckCircle className="w-3 h-3" />{seriesItem.freeCount} frei
                            </span>
                            {seriesItem.blockedCount > 0 && (
                              <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">
                                <AlertTriangle className="w-3 h-3" />{seriesItem.blockedCount} Konflikt{seriesItem.blockedCount !== 1 ? 'e' : ''}
                              </span>
                            )}
                          </div>

                          {rejectDialog?.seriesId === seriesItem.seriesId && !rejectDialog?.bookingId && (
                            <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                              <label className="block text-sm font-medium text-red-800 mb-2">Grund der Ablehnung (optional):</label>
                              <textarea value={rejectDialog.reason}
                                onChange={(e) => setRejectDialog({ ...rejectDialog, reason: e.target.value })}
                                placeholder="z.B. Ressource bereits anderweitig vergeben..." rows={2}
                                className="w-full px-3 py-2 border border-red-300 rounded-lg focus:ring-2 focus:ring-red-500 text-sm" />
                              <div className="flex gap-2 mt-2">
                                <button onClick={() => handleReject(seriesItem.seriesBookings[0].id)}
                      ...
                      {!(rejectDialog?.seriesId === seriesItem.seriesId) && (
                        <div className="flex gap-2 ml-4 flex-shrink-0">
                          <button onClick={() => handleApproveWrapped(seriesItem.seriesBookings[0].id)}
                          ...
                          <button onClick={() => setRejectDialog({ seriesId: seriesItem.seriesId!, reason: '' })}
                            disabled={!!processingId}
                    ...
                    <div className="border-t border-gray-100 bg-gray-50">
                      <div className="px-4 py-2">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-2">
                          Einzeltermine ({seriesItem.totalCount})
                        </div>
                        <div className="space-y-1">
                          {seriesItem.seriesBookings.map(sb => {
                              ...
                              <div key={sb.id}
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  {hasConflict && !showingSbReject && (
                                    <div className="flex gap-1 ml-auto flex-shrink-0">
                                      ...
                                      <button onClick={() => setRejectDialog({ bookingId: sb.id, seriesId: seriesItem.seriesId!, reason: '' })}
                                        disabled={!!processingId}
                                ...
                                {showingSbReject && (
                                  <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded">
                                    <textarea value={rejectDialog!.reason}
                                      onChange={(e) => setRejectDialog({ ...rejectDialog!, reason: e.target.value })}
                                      placeholder="Grund (optional)..." rows={1}

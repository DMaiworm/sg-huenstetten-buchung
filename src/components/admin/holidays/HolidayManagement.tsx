...
const TYPE_LABELS: Record<string, string> = { feiertag: 'Feiertage', schulferien: 'Schulferien' };
...
interface HolidayManagementProps {
  ...
  createHolidaysBulk?: (data: any[]) => Promise<{ error?: string | null }>;
  updateHoliday?: (data: any) => Promise<any>;
  deleteHoliday?: (id: string) => Promise<any>;
  deleteHolidaysByYear?: (year: number, type?: string) => Promise<any>;
}
...
const HolidayManagement: React.FC<HolidayManagementProps> = ({
  ...
  const toggleYear = (year: number) => setCollapsedYears(p => ({ ...p, [year]: !p[year] }));
  ...
  const handleDeleteYear = async (year: number, type?: string) => {
  ...
  const handleImportDone = async (importedHolidays: any[]) => {
  ...
  return (
    <div>
      <PageHeader icon={CalendarDays} title="Ferien & Feiertage" subtitle={subtitle}
        actions={
          <div className="flex items-center gap-2">
            <Button variant="secondary" size="sm" onClick={() => setShowImport(true)}><Download className="w-4 h-4 mr-1" /> Importieren</Button>
            <Button variant="primary" size="sm" onClick={() => { setEditingHoliday(null); setShowAddForm(true); }}><Plus className="w-4 h-4 mr-1" /> Neuer Eintrag</Button>
          </div>
        }
      />
      ...
      {years.length === 0 ? (
        <EmptyState icon={CalendarDays} title="Keine Ferien & Feiertage vorhanden"
          subtitle="Importieren Sie Feiertage und Schulferien oder erstellen Sie Einträge manuell."
          action="Jetzt importieren" onAction={() => setShowImport(true)} />
      ) : (
        years.map(({ year, items }) => {
          ...
          return (
            <div key={year} className="mb-6 border border-gray-200 rounded-xl overflow-hidden bg-white">
              <div className="flex items-center justify-between px-4 py-3 bg-gray-50 border-b border-gray-200 cursor-pointer select-none"
      ...
      <HolidayFormModal open={showAddForm} onClose={() => { setShowAddForm(false); setEditingHoliday(null); }}
        onSave={handleSaveHoliday} holiday={editingHoliday} />
      <HolidayImportModal open={showImport} onClose={() => setShowImport(false)}
        onImport={handleImportDone} existingHolidays={holidays} />
      {confirmDialog}
    </div>
  );
};

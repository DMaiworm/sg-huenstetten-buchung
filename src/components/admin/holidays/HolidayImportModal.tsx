...
const HESSEN_FEIERTAGE_NAMES: Record<string, string> = {
...
const FERIEN_NAMES: Record<string, string> = {
...
async function fetchFeiertage(year: number): Promise<ImportItem[]> {
  ...
  return data
    .filter((h: any) => !h.counties || h.counties.includes('DE-HE'))
    .map((h: any) => ({
      name: HESSEN_FEIERTAGE_NAMES[h.name] || h.localName || h.name,
      type: 'feiertag', startDate: h.date, endDate: h.date, year,
    }));
}

async function fetchSchulferien(year: number): Promise<ImportItem[]> {
  ...
  return data.map((f: any) => {
    ...
    return { name: ferienName, type: 'schulferien', startDate, endDate: adjustedEnd, year };
  });
}
...
const HolidayImportModal: React.FC<HolidayImportModalProps> = ({ open, onClose, onImport, existingHolidays }) => {
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      if (importFeiertage) results.push(...await fetchFeiertage(selectedYear));
      if (importSchulferien) results.push(...await fetchSchulferien(selectedYear));
      const filtered = results.filter(r =>
        !existingHolidays.some(e => e.name === r.name && e.startDate === r.startDate && e.year === r.year)
      );
  ...
  const handleClose = () => {
    setPreview(null); setError(null); setImported(false);
    onClose();
  };
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Ferien & Feiertage importieren" maxWidth="max-w-2xl"
      footer={
        <>
          <Button variant="ghost" onClick={handleClose}>{imported ? 'Schließen' : 'Abbrechen'}</Button>
          {!imported && !preview && (
            <Button variant="primary" onClick={handlePreview} disabled={loading || (!importFeiertage && !importSchulferien)}>
        ...
        <>
          ...
          {(hasFeiertage || hasFerien) && (
            <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700">
              ...
              {hasFeiertage && 'Feiertage'}{hasFeiertage && hasFerien && ' und '}{hasFerien && 'Schulferien'}.
          ...
          <div className="mb-5 text-xs text-gray-400">Datenquellen: date.nager.at (Feiertage) · ferien-api.de (Schulferien)</div>
          {error && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700 flex items-start gap-2">
              <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
              <div><strong>Fehler beim Laden:</strong> {error}<p className="mt-1 text-xs">Bitte prüfen Sie Ihre Internetverbindung und versuchen Sie es erneut.</p></div>
            </div>
          )}
          {preview && (
            <div>
              {preview.duplicates > 0 && <p className="text-sm text-amber-600 mb-3">{preview.duplicates} bereits vorhandene Einträge werden übersprungen.</p>}
              {preview.filtered.length === 0 ? (
                <div className="text-center py-6 bg-gray-50 rounded-lg"><p className="text-sm text-gray-500">Alle Einträge sind bereits vorhanden.</p></div>
              ) : (
                      ...
                      {preview.filtered.map((h, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          <td className="px-3 py-1.5 text-gray-600 font-mono text-xs whitespace-nowrap">
                            {h.startDate === h.endDate ? formatDate(h.startDate) : `${formatDate(h.startDate)} – ${formatDate(h.endDate)}`}
                          </td>
                          <td className="px-3 py-1.5 text-gray-900">{h.name}</td>
                          <td className="px-3 py-1.5">
                            <span className={`text-xs px-2 py-0.5 rounded-full ${h.type === 'feiertag' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>

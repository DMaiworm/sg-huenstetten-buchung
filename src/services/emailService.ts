...
import type { EmailData, SentEmail, Booking, BookableResource } from '../types';
...
interface EmailApprover {
  firstName: string;
  lastName: string;
}

interface EmailResult extends EmailData {
  status: string;
  sentAt: string;
}
...
export const EMAIL_TEMPLATES = {
  bookingCreated: (booking: Booking, user: EmailUser, resource: BookableResource): EmailData => ({
    to: user.email,
  ...
  bookingApproved: (booking: Booking, user: EmailUser, resource: BookableResource, approver: EmailApprover): EmailData => ({
    to: user.email,
  ...
  bookingRejected: (booking: Booking, user: EmailUser, resource: BookableResource, approver: EmailApprover, reason: string | null): EmailData => ({
    to: user.email,
  ...
  adminNewBooking: (booking: Booking, user: EmailUser, resource: BookableResource, adminEmail: string): EmailData => ({
    to: adminEmail,
...
export class EmailService {
  ...
  async send(emailData: EmailData): Promise<EmailResult> {
    try {
      ...
      return { ...emailData, status: (result as { status?: string })?.status || 'sent', sentAt: new Date().toISOString() };
    } catch (err) {
  ...
  async getSentEmails(): Promise<SentEmail[]> {
    try {
      ...
      return (data || []).map((e: Record<string, unknown>) => ({
        id: e.id as string,
        to: e.recipient as string,
        subject: e.subject as string,
        html: e.html_body as string,
        status: e.status as string,
        sentAt: e.created_at as string,
        errorMessage: (e.error_message as string | null) || null,
      }));
    } catch (err) {
      console.warn('E-Mail-Log nicht geladen:', (err as Error).message);
      return [];
    }
  }
  ...
  async clearEmails(): Promise<void> {
    try {
      ...
      console.warn('E-Mail-Log leeren fehlgeschlagen:', (err as Error).message);
    }
  }
}
